...
!!! Also an important thing to notice: Since I have for instance only one 24V DI (digital input) for starting the motor, 
it should be controlled so that if manual start is HIGH, motor starts, if LOW motor is stops. 
...
So I implement priority switching. And if priority is changed, motor holds the last value.


Last words being said, if you want to extend the control system, 
